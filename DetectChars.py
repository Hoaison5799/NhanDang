...
MAX_DIAG_SIZE_MULTIPLE_AWAY =  2.6 #2.3
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        if Main.showSteps == True: # show steps ###################################################
            name = "BienDoi/5a - " + str(dem) + ".jpg"
            cv2.imwrite(name, possiblePlate.imgPlate)
            name = "BienDoi/5b - " + str(dem) + ".jpg"
            cv2.imwrite(name, possiblePlate.imgGrayscale)
            name = "BienDoi/5c - " + str(dem) + ".jpg"
            cv2.imwrite(name, possiblePlate.imgThresh)
        # end if # show steps #####################################################################
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            imgContours = np.zeros((height+200, width+200, 3), np.uint8)
            ...
            name = "BienDoi/6 - " + str(dem) + ".jpg"
            cv2.imwrite(name, imgContours)
        # end if # show steps #####################################################################

                # given a list of all possible chars, find groups of matching chars within the plate
        listOfPossibleCharsInPlate.sort(key=lambda matchingChar: matchingChar.intCenterX)  # sort chars from left to right
        height, width, numChannels = possiblePlate.imgPlate.shape
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            for listOfMatchingChars in listOfListsOfMatchingCharsInPlate:
                ...
                del contours[:]
                contours.append(listOfMatchingChars[0].contour)
                 

                # end for
                name = "BienDoi/7 - " + str(dem) + ".jpg"
                cv2.imwrite(name, imgContours)
        ...
        if Main.showSteps == True: # show steps ###################################################
            ...
            cv2.drawContours(imgContours, contours, -1, Main.SCALAR_WHITE)
            name = "BienDoi/9 - " + str(dem) + ".jpg"
            cv2.imwrite(name, imgContours)
        # end if # show steps #####################################################################

        possiblePlate.strChars = recognizeCharsInPlate(possiblePlate.imgThresh, longestListOfMatchingCharsInPlate, dem)
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars, dem):
    ...
    if Main.showSteps == True: # show steps #######################################################
        name = "BienDoi/10 - " + str(dem) + ".jpg"
        cv2.imwrite(name, imgThreshColor)
